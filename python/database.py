...
def getArticles(query, number=None):
    if number is not None: articles = [a for a in db.article.find(query).sort([("date", DESCENDING)]).limit(number)]
    else: articles = [a for a in db.article.find(query).sort([("date", DESCENDING)])]
